...
#  library(arules)
 ...
 #attempting apriori
  ...
  #library(wordcloud)
  #wordcloud(textMoreThan10Freq$Word, random.order = FALSE, max.words = 300, col = brewer.pal(9,"Blues"),random.color = TRUE)
...
freq<-50
raw.text<-scan("largeTxt.txt",what="char",sep="")
#head(raw.text)
#converting to lower case
raw.text<-tolower(raw.text)
#raw.text <- tm_map(raw.text, removeWords, stopwords("english"))
...
wordcloud(textMoreThan10Freq$Word, random.order = FALSE, max.words = 1500, col = brewer.pal(9,"Blues"),random.color = TRUE)
